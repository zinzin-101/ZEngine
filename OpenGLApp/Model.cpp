...
Model::Model(const std::string& path, bool gamma): gammaCorrection(gamma), tiling(1.0f) {
	loadModel(path);
}
...
void Model::draw(Shader& shader) {
	...
	for (size_t i = 0; i < n; i++) {
		meshes.at(i).draw(shader, tiling);
	}
}
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene) {
    ...
    // retrieve the corresponding vertex indices for each triangle.
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial* material, aiTextureType type, std::string typeName) {
    ...
    for (unsigned int i = 0; i < material->GetTextureCount(type); i++)
    {
        ...
        {
            if (std::strcmp(loadedTextures[j].path.data(), str.C_Str()) == 0 && typeName == loadedTextures[j].type)
            {
